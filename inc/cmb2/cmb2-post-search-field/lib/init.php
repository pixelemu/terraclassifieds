...
class CMB2_Post_Search_field {
	...
	public function render_field( $field, $escaped_value, $object_id, $object_type, $field_type ) {
		...
		echo '<input onkeydown="return false;" style="caret-color: transparent !important; pointer-events: none; background: #ddd;" type="text" class="post-titles" value="' . implode( ', ', $titles ) . '" />';
		...
		echo $field_type->input( array(
			...
			'data-search' => json_encode( array(
				...
				'findtxt'    => $field->args( 'search_text' ) ? esc_attr( $field->args( 'search_text' ) ) : esc_attr( $field_type->_text( 'find_text', __( 'Find Page' ) ) ),
